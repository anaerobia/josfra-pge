...
intn PTchkptid(int32, const char *, int32 *, int32 *, int32 *);
intn PTlinkinfo(int32, int32, int32, const char *mode, char *);
...
intn PTwrrdattr(int32, char *, int32, int32, const char *, VOIDP);
...
-----------------------------------------------------------------------------*/
intn
PTchkptid(int32 pointID, const char *routname,
...
-----------------------------------------------------------------------------*/
int32
PTsizeof(int32 pointID, const char *fieldlist, int32 fldlevels[])
{
    intn            i;		/* Loop index */
    intn            j;		/* Loop index */
    intn            status = 0;	/* routine return status variable */
	...
	if (status == 0)
	{
	    char *tmpfieldlist=NULL;
	    /* Parse field list */
	    /* ---------------- */
	    nflds = EHparsestr(fieldlist, ',', pntr, slen);
	    /*
	    tmpfieldlist = (char *)malloc(((int)strlen(fieldlist)+1)*sizeof(char));
	    strcpy(tmpfieldlist, fieldlist);
	    nflds = EHparsestr(tmpfieldlist, ',', pntr, slen);
	    free(tmpfieldlist);
	    tmpfieldlist=NULL;
	    */
...

{
	...
	if (status == 0)
	{
		...
		VSwrite(vdataID, zerop, 1, FULL_INTERLACE);
		free(zerobuf);
		...
		if (nlevels > 0)
		{
		    ...
		    /* -------------------------------------- */
		    size = EHsizeof(vdataID, "BCKPOINTER");
		    ...
		    /* ---------------------------- */
		    snprintf(utlbuf, sizeof(utlbuf), "%s%d%s%d", "BCKPOINTER:", nlevels,
			    "->", nlevels - 1);
		    ...
		    /* -------------------------------------- */
		    size = EHsizeof(vdataID, "BEGIN,EXTENT");
		    ...
		    /* ---------------------------- */
		    snprintf(utlbuf, sizeof(utlbuf), "%s%d%s%d", "FWDPOINTER:", nlevels - 1,
			    "->", nlevels);
		    VSsetname(vdataID, utlbuf);
	...
	if (status == 0)
	  {
	    ...
	    /* ------------------------------------------ */
	    snprintf(utlbuf, sizeof(utlbuf), "%s%s%s%s%s", parent, "/", child, ":", linkfield);
...
PTlinkinfo(int32 pointID, int32 sdInterfaceID, int32 level,
	   const char *mode, char *linkfield)
{
    intn            status = 0;	/* routine return status variable */
    ...
    /* ---------------------------- */
    snprintf(utlbuf, sizeof(utlbuf), "%s%s", "\t\t\t\tParent=\"", name1);
...
PTwrbckptr(int32 pointID, int32 level, int32 nrec, int32 recs[])
{
	...
	if (level > 0)
	{
		...
		/* -------------------------- */
		snprintf(utlbuf, sizeof(utlbuf), "%s%d%s%d",
...
PTrdfwdptr(int32 pointID, int32 level, int32 nrec, int32 recs[])
{
	...
	/* Get ID of FWDPOINTER vdata */
	/* -------------------------- */
	snprintf(utlbuf, sizeof(utlbuf), "%s%d%s%d", "FWDPOINTER:", level,
		"->", level + 1);
...
-----------------------------------------------------------------------------*/
intn
PTreadlevel(int32 pointID, int32 level, const char *fieldlist,
...

{
	...
	if (status == 0)
	{
	    ...
	    tmpfieldlist = (char *)malloc(((int)strlen(fieldlist)+1)*sizeof(char));
	    strcpy(tmpfieldlist, fieldlist);
	    nfields = EHparsestr(tmpfieldlist, ',', pntr, slen);
	    free(tmpfieldlist);
	    tmpfieldlist=NULL;
	    */
	    /* Check that all fields in list exist in level */
