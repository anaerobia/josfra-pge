...
FILENAME:
	PGS_PC_GetFileByAttrF.c

DESCRIPTION:
	This file contains the function PGS_PC_GetFileByAttr().
	This version contains the FORTRAN binding.
...
HISTORY:
 	06-Jul-95 RM/MES/GTSK  Move FORTRAN bindings to PGS_PC_GetFileByAttrF.c
			Copied from PGS_PC_GetFileByAttr.c.
	14-Jul-95 RM Changed input parameter from a pointer to a function 	
			to a void * to remove warning message as suggested
			by Guru Tej Khalsa.
...
DETAILS:
        ...
        FUNCTION PGS_PC_GetFileByAttr().  PGS_PC_GetFileByAttr() IS THE C
        COUNTERPART TO PGS_PC_GetFileByAttrF().
...
PROTOCCALLSFFUN1(INT,SEARCHFUNC,searchfunc,PSTRING)

#define       searchFunc(ATTRIBUTE)       \
CCALLSFFUN1(SEARCHFUNC,searchfunc,PSTRING,ATTRIBUTE)


PGSt_SMF_status
PGS_PC_GetFileByAttrF(                  /* get file by attribute */
    PGSt_PC_Logical   prodID,	       /* logical value */
    void             *searchFuncF,     /* user supplied function */
...
***************************************************************************/
    ...
    searchFunc = (PGSt_integer (*)(char*)) searchFuncF;
...
printf("back from searchFunc - foundFlag = %d\n",foundFlag);
...
FCALLSCFUN4(INT,PGS_PC_GetFileByAttrF,PGS_PC_GETFILEBYATTR,pgs_pc_getfilebyattr,
           INT,ROUTINE,INT,PINT)
