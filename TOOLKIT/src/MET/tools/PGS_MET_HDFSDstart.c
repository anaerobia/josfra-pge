  ...
  if(FILE_IS_HDF5 == 1) /* file is HDF5 type */
    {
      ...
      if( status != -1) /* file is closed; Set open_flag to 0. Also set file_id to -1, so that
			   the newly opened files, which will have the ID of the closed file, are not
			   mistaken by the closed file */
	{
	  /*  strcpy (cat_HDF5[filecount].file_name,"");*/

	  cat_HDF5[filecount].open_flag = 0;
	  cat_HDF5[filecount].file_id = -1;
	}
      if( status == -1)
	{
