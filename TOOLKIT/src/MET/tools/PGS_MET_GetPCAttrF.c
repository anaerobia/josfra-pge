...
HISTORY:
	...
	11-MAR-96      ANS     Updated for tk5+
        09-Apr-97      CSWT    Added code so that landsat7 files and the different
        ...
        02-Feb-98      CSWT    Added C library function free() to release previously allocated
                               ...
                               _get_pcattrib)
        02-Feb-98      CSWT    Added C Standard I/O function "fclose" to close the file associated  
	...
	07-Jul-99      RM      update for TSF functionality
	25-Aug-2000    AT      corrected the leap year calculation
...
SYNOPSIS:
C:
	N/A. Only to be used through fortran interface. See PGS_MET_GetPCAttr() for more details
FORTRAN:
  	N/A 
DESCRIPTION:
...
RETURNS:   
	...
	PGSMET_E_ODLTOVAL_ERR   Unable to convert attribute values from the odl format
				error returns from lower level routines should expalin the problem
	PGSMET_E_DD_UNKNOWN_PARM Parameter is not found in the hdf file
				
	PGSMET_E_DD_UNKNOWN_PARM_ATTR Parameter value is not defined i.e. there is no associated value field
...
NOTES:
	...
	since user is expected to know n for setting the return buffer.
        ...
        now simply retrieves the values from the HDF headers and does not perform type and range checking. The
       ...
       unsigned int 	stringSize)			/* size of fortran strings send as a hidden argument */
{
	...
	char *			errInserts[PGSd_MET_MAX_ERR_INSERTS] = {NULL}; 
							 ...
							 * error messages
							 */
	char *			funcName = "PGS_MET_GetPCAttrF";
	...
	/* static char             attrHandle[PGSd_MET_NUM_OF_SIG][PGSd_MET_SIGNATURE_L] = {""}; */
    ...
    AGGREGATE *PGSg_MET_AggList;
    char *PGSg_MET_AttrHandle;
    AGGREGATE *PGSg_MET_MasterNode;
    PGSt_TSF_MasterStruct *masterTSF;
    int masterTSFIndex;
    extern PGSt_integer PGSg_TSF_METnewFilePositionF[];

    /* get struct holding TSD keys and get master global index */
    ...
    /* assign from global counterpart and TSDs */
    newFilePosition = PGSg_TSF_METnewFilePositionF[masterTSFIndex];
    PGSg_MET_MasterNode = (AGGREGATE *) pthread_getspecific(
                                    masterTSF->keyArray[PGSd_TSF_KEYMETMASTERNODE]);
    PGSg_MET_AggList = (AGGREGATE *) pthread_getspecific(
	...
	/* clear the errno for the ODL routines */
         ...
         * productmetadata.X or archivemetadata.X,where .x is a squeence number
         *beginning at 0 and running as high as 9
         */
	...
	/* prepare a unique signature */
        ...
        bufferIndex = 0;
#ifdef _PGS_THREADSAFE
        /* TSF data has different types so we differentiate */
	...
	else /* attributes are contained in a separate file */
	{
	   ...
	   parmNode = FindParameter(aggNode, name);
	   ...
	   else
           {
           	...
           	if(valueNode->item.type == TV_STRING || valueNode->item.type == TV_SYMBOL)
           	{
				...
				/* fill the array with blanks first */
                        	blankPtr = strPtr1; /* point to the first location of individual string */
                        	...
                        	{
                                	strPtr1[charcterCount] = valueString[charcterCount];
                                	charcterCount++;
                        	}
                        	strPtr1 = strPtr1 + stringSize; /* point to the next array string */
                ...
                else if(valueNode->item.type == TV_DATE || valueNode->item.type == TV_TIME || valueNode->item.type == TV_DATE_TIME || valueNode->item.type == TV_SYMBOL)
                {
                        ...
                        while(valueNode != NULL)
                        {
                                ...
                                free(date_time_Ptr);
                                datetimePtr = datetimePtr + stringSize; /* point to the next array string */ 
                                valueNode = NextValue(valueNode);
                        }
           	...
           	else
           	{
                	...
                	while(valueNode != NULL)
                	{
                        	*dblPtr = (PGSt_integer)valueNode->item.value.real.number;
                        	dblPtr++;
                        	valueNode = NextValue(valueNode);
                	}
           	}
		...
		if(fileInBuffer != 0) /* its a new file or attribute */
                {
#ifdef _PGS_THREADSAFE
                        /* datatype different in TSF mode */
...
#ifdef _PGS_THREADSAFE
                        /* re-set global and TSD values */
                        PGSg_TSF_METnewFilePositionF[masterTSFIndex] = 
                                                              newFilePosition;
                        pthread_setspecific(
                                masterTSF->keyArray[PGSd_TSF_KEYMETAGGLIST],
                                PGSg_MET_AggList);
	...
	*/
	...
	aggNode = PasteAggregate(PGSg_MET_MasterNode[0], aggNode);
        ...
        if(hdfAttrName != NULL || *hdfAttrName !='\0')
        {
           if(strlen(hdfAttrName) > (strlen(PGSd_MET_GROUP_STR) + 1))
           {
                 if(strncmp(hdfAttrName, PGSd_MET_GROUP_STR, strlen(PGSd_MET_GROUP_STR))
== 0)
...
#ifdef _PGS_THREADSAFE
        /* unlock here because PGS_MET_GetSetAttrF uses same lock */
        retTSF = PGS_TSF_UnlockIt(PGSd_TSF_LOCKODL);
#endif
	retVal = PGS_MET_GetSetAttrF(locAggrName, parmName, parmValue, stringSize);
...
#ifdef _PGS_THREADSAFE
             /* re-set global and TSD value */
             PGSg_TSF_METnewFilePositionF[masterTSFIndex] = newFilePosition;
