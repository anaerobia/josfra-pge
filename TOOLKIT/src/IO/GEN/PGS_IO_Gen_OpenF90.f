...
!     24-Jan-2002 AT  added action='read' to file opening with READ access
!                     per NCR ECSed21811, to resolve problems with opening
!                     files on a cross mounted host (with READ ONLY access).
! 
! END_FILE_PROLOG
!***************************************************************************
...
!     PGS_IO_Gen_OpenF.f . Differences are highlighted by the 
!     string "***F90 SPECIFIC***".
!     At installation time, this code module is compiled if
!     the user specifies Fortran 90 as his/her language choice;
!     otherwise, code module PGS_IO_Gen_OpenF.f is compiled.
!     The function name "PGS_IO_Gen_OpenF" is the same in both cases.
! 
! REQUIREMENTS: 
!     PGSTK-0360
! 
...
!     ***F90 SPECIFIC *** (recl keyword)
     ...
     +        recl = record_length, action='read', err = 500)
         else
            open (unit = lun, file = outstr, form = 'formatted',
     +        access = 'sequential', status = 'old', 
     +        action='read', err = 500)
...
!     PDN changed integer*4 to integer below due to sgi new 32 bit
!     compiler diagnostic (warning) . I believe offender was maxlength
 
      integer function GET_String_Length ( char, maxlength )
 
      implicit       none
      integer      i, istop, maxlength
