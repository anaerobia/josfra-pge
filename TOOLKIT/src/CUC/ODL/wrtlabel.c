/*****************************************************************************
  ...
  17-Nov-2008  Abe Taaheri  Modified this function to prevent strings from out
                            of bound writes. To return error if the length of
                            attribute string is larger than the dynamically
                            allocated memory. In Toolkit this may happen for 
                            INPUT POINTER where string may contain thousands of
                            filenames such that the string length exceeds
                            ODLMAXSTMT = 255001. 
       ...
       AGGREGATE  base_node)
{
  ...
  int        len0,len1;         /* temporay length of stmt string           */
     ...
     while (parameter != NULL)
       {
         ...
         while (data != NULL)
		   ...
		   len0=len;
                   if (parameter->node_kind == KP_POINTER)
                   {
		     /* get the length of string to be added in next call to
			ODLFormatString
		     */
		     len1=ODLFormatStringlen(&data->item, &ncol,
					nindent+2*INDENTSIZE, MAXLABLINE-2, 1, 1);
		     /* Since the memory overflow may only occur for KV_SEQUENCE 
			type strings in writing INVENTORY metadata, where there 
			is possibility that string length excceds 
			ODLMAXSTMT= 255001, we use 3 below (instead of 2) to count 
			for the closing paranthesis.
		     */ 
		     if((len0+len1+3) > ODLMAXSTMT)
		       {
			 free(stmt);
			 return -1;
		       }
