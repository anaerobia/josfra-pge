/*********************************************************
PGS_DEM_ReplaceFillPointsFlt32.c--
...
corner pixels of the region were checked to see if they were reasonable.
 ...
 This Function only replaces fill points for layers with data types of FLOAT32 
...
11/20/1997   EMT      Add wrapping condition, Nearest Neighbor, 
...
{
    /*pointer to switch dataRegionIn to float32 pntr. */
    float32 *dataRegion;
    ...
    dataRegion = (float32 *) dataRegionIn;
	...
	for (i = 0; i < totalNumPixels; i++)
	{
	    if (dataRegion[i] == (float32) (subsetInfo -> fillvalue))
	    {
		numFillPoints ++;
