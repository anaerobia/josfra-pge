...
orders the indices based on the subgrid value of the _PointRecord.  Essentially,
 it reorders a list of the indices of PGS_DEM_PointRecords into groupoings by
...
3/24/99        AT       Changed the function PGS_DEM_CompareIndicesBySubgrid
...
/*This isn't quite kosher, but come back and fix when implement new searching
  routine*/
    ...
    PGSt_DEM_PointRecordDeg **SumdegpointArray;
#else
    static PGSt_DEM_PointRecordDeg **pointArray;
#endif 
...
PGS_DEM_OrderIndicesSumDeg(
    ...
    PGSt_DEM_PointRecordDeg **dataArray,  /*Array containing subgrid number*/
...

{
    ...
    int PGS_DEM_CompareIndicesBySubgrid(const void *, const void *);
    ...
    SumdegpointArray = (PGSt_DEM_PointRecordDeg **) pthread_getspecific(
                              masterTSF->keyArray[PGSd_TSF_KEYDEMSUMDEGPOINTARRAY]);
#endif
...
#ifdef _PGS_THREADSAFE
    SumdegpointArray = dataArray;
    /* Reset TSD key */
    pthread_setspecific(masterTSF->keyArray[PGSd_TSF_KEYDEMSUMDEGPOINTARRAY],
                            SumdegpointArray);
#else
...
int PGS_DEM_CompareIndicesBySubgrid(const void *qIndicesPointer, 
					   const void *yIndicesPointer)
{
#ifdef _PGS_THREADSAFE
    return( (SumdegpointArray[*(int *)qIndicesPointer] -> subgridValue) - 
	    (SumdegpointArray[*(int *)yIndicesPointer] -> subgridValue));
#else
...
/*comparison function for qsort*/
int PGS_DEM_CompareIndicesBySubgrid_orig(const void *qIndicesPointer, 
					   const void *yIndicesPointer)
{
...
#ifdef _PGS_THREADSAFE
    subgridValue1 = SumdegpointArray[*aIndicesPointer] -> subgridValue;
    subgridValue2 = SumdegpointArray[*bIndicesPointer] -> subgridValue;
#else
