...
HISTORY:
  	...
  	11-Aug -94 	GJB     Update for coding standards
        31-August-94    GJB     Post code review updates 
	03-October-94   GJB     Create F version of function
	25-January-95  	SGT	Added "undef _CFE" to solve compile problem on SGI IRIX 5.2
        16-Feb-95       GJB     Add frees
	23-Jun-95       ANS     Changed val to static
	23-Jun-95	ANS     Improved fortran example in prolog
	19-July-95	ANS	Fixed ECSed00779
...
FORTRAN:
	 ...
	 character*99 iparms(*)
         ...
         integer function pgs_aa_3dgeo( parms, nparms, latitude, longitude, 
...
INPUTS:
	...
	----		-----------		-----	---	---
        iparms          parameter names        	see notes
                        requested
        ...
        longitude       longitude(s) of the     degrees -180.00 180.00
                        requested point
        height          height of the           none    1       variable
                        requested point
...
C:
     ...
     char iparms[][100] = { "etop05SeaLevelElevM" };
     long nParms = 1;
     ...
     retStatus = PGS_AA_3Dgeo(parms, nParms, latitude, longitude, height,
          nPoints, fileId, version, operation, results);
             
FORTRAN:
	IMPLICIT     NONE
	integer      pgs_aa_3dgeo
	character*99 iparms(4)
        integer      nParms
        real*8       latitude(300)
...
NOTES:
	...
	Height or the z dimension is a layer number in the file and 
			...
			void *results)
{
    ...
    short toolUsed,i;                                 /* specifies 2 or 3 D tool used */
    PGSt_SMF_status val;
    ...
    if (parmBuffer == NULL)
    {
        PGS_SMF_SetStaticMsg (PGSAA_E_MALLOC,"PGS_AA_3Dgeo");
    ...
    /* set tool used to 3DGeo */
    toolUsed = TOOL3D;
    
    /* extract 3d parameters */

    val = PGS_AA_GEOGrid(parms, nParms, logSuppFile, operation, 
    ...
    PGS_SMF_SetStaticMsg (PGS_S_SUCCESS,"PGS_AA_3Dgeo");
    return PGS_S_SUCCESS;    
}
/*
#define pgs_aa_3dgeo_STRV_A1 NUM_ELEMS(4)
FCALLSCFUN10(INT,PGS_AA_3Dgeo,PGS_AA_3Dgeo,pgs_aa_3dgeo,STRINGV,INT,DOUBLEV,DOUBLEV,INTV,INT,INT,INT,INT,PVOID)  
*/
