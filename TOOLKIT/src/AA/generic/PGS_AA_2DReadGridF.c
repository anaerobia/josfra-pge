...
HISTORY:   
	...
	30-Sep-94       GJB     Change to long output to cope with DEC and create
	                        this new F version with appropriate casting/types
			   ...
			   
{
    ...
    PGSt_integer *tempShortResult;           /* temporary type specific pointer/array used to fill results memory */
    PGSt_integer *tempLongResult;     /* temporary type specific pointer/array used to fill results memory */
...
/* first check start points are not out of range */
    ...
    {
	tempShortResult = (PGSt_integer*)results;
	...
	for ( yCount = 0; yCount < yDim; yCount++)
	{
		...
		tempShortResult[count] = (PGSt_integer)*(shortPointer + xCount + 
					   (yCount * xCells * nParms));
    ...
    {
	tempLongResult = (PGSt_integer*)results;
	...
	for ( yCount = 0; yCount < yDim; yCount++)
	{
		...
		tempLongResult[count] = (PGSt_integer)*(longPointer + xCount + (yCount * xCells * nParms));
