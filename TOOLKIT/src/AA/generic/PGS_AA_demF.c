...
HISTORY:
        19-JAN-94      ANS     Initial version
	09-Feb-95      GJB     Add CFE for SGI 5.2 compatibility
	15-Feb-95      ANS     Fix DR 00701
	16-Feb-95      ANS     Fixed DR 00746
	17-Feb-95      ANS     Fixed so that missing points are reported
                               if at the beginning or end of data array
	17-Feb-95      ANS     Fixed so that float and doubles are read
			       correctly on the dec
	17-Feb-95      ANS     Changed shortDataSise to shortDataSize
	...
	21-July-95      ANS     explained byteswapping and dec long datatype problem in the prolog
        11-Sep-97     CSWT     Fixed bug ECSed06668 the HP is experienceing different results for
                               2 AA_dem tests 
...
REQUIREMENTS:
	PGSTK-0980 PGSTK-1000 PGSTK-1030
...
***************************************************************************/
PGSt_SMF_status
PGS_AA_demF(				/* fetches DEM values for the given set of points */
     ...
     
{
    ...
    char swapBytes[10];
    char temp[20] = {0}; 
    PGSt_integer shortDataSize = 0;
    PGSt_integer tempResult = 0;
#endif
   ...
   /* DEM file may be tiled so get the number of file */
	...
	{
	    dataSize = sizeof(PGSt_double);
        }
        else if(strcmp("short", dataType[0]) == 0)
        {
            dataSize = sizeof(PGSt_integer);
        }
     ...
     */
        ...
        for (firstCount = 0; firstCount < nPoints; firstCount++)
        {
		    ...
		    retVal = PGS_AA_2DgeoF(localParms, nParms, &latitude[startPoint], 
...
#ifdef dec
/* in case the data was short and is casted to integer, i.e 4bytes, only
   the first two bytes need to be swapped for eg the data returned from
   2DGEO for a given point is (in terms of bytes) 2 -97 -1 -1, it should 
   be swapped to read -97 2 0 0
 */
    ...
    {
      shortDataSize = dataSize;
      if(strcmp(dataType[0], "short") == 0)
      {
	 shortDataSize = sizeof(short);
      }
      for(firstCount = 0; firstCount < (nPoints * nParms); firstCount++)
      {
        /* transfer individual bytes into an temp array */
        for(secondCount = 0; secondCount < shortDataSize; secondCount++)
        {
            resultBytePtr = (char *) ((char *)results + ((firstCount * dataSize) + secondCount));
            temp[shortDataSize - secondCount - 1] = *resultBytePtr;
        }
	...
	tempResult = *((int *) results + firstCount);
	if(tempResult > (int) 32765 && shortDataSize == sizeof(short))
	{
	    *((int *) results + firstCount) = tempResult - (int) 65536;
	}
      }
    }
#endif
