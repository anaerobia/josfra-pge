...
HISTORY:
	...
	03-October-94   GJB     Make F verion
			  ...
			  void *results)  
{
    ...
    PGSt_double  locResults = 15.0;	/* actual results calculated by the user routine */
    PGSt_double  roundOff;      /* to correctly round off the variable */
    short *	 shortPtr = NULL;	/* short pointer to manage user result buffer if type is short */ 
	...
	/* perform interpolation on 2x2 grid */
	    ...
	    x1a[0] = (PGSt_double) floor(xRealStart + (double)0.5);
	    ...
	    x2a[0] = (PGSt_double) floor(yRealStart + (double)0.5);
	   ...
	   if(strcmp("short", dataType[0]) == 0)
           {
		shortPtr = (short *) yBuffer;
		y2d[0][0] = (PGSt_double) *shortPtr;
		y2d[0][1] = (PGSt_double) *(shortPtr + 1);
		y2d[1][0] = (PGSt_double) *(shortPtr + 2);
		y2d[1][1] = (PGSt_double) *(shortPtr + 3);
           }
	   ...
	   if(strcmp("short", dataType[0]) == 0)
    	   {
        	shortPtr = (short *) results;
                *shortPtr= (short)(locResults + roundOff);
    	   }
    	   else if(strcmp("long", dataType[0])==0)
    	   {
	...
	/* perform interpolation on 3x3 grid */
		...
		y2d[0][0] = (PGSt_double) *shortPtr;
		y2d[0][1] = (PGSt_double) *(shortPtr + 1);
		y2d[0][2] = (PGSt_double) *(shortPtr + 2);
		y2d[1][0] = (PGSt_double) *(shortPtr + 3);
		y2d[1][1] = (PGSt_double) *(shortPtr + 4);
		y2d[1][2] = (PGSt_double) *(shortPtr + 5);
		y2d[2][0] = (PGSt_double) *(shortPtr + 6);
		y2d[2][1] = (PGSt_double) *(shortPtr + 7);
		y2d[2][2] = (PGSt_double) *(shortPtr + 8);
           }
           else if(strcmp("long", dataType[0])==0)
           {
		longPtr  = (long *) yBuffer;
