 ...
 *				       char * query_str ) ;
 *
 *    int read_gazetteer_index_directory (
 *					ThematicIndexDirectory ** gid,
 *					ThematicIndexHeader     * gi,
 *					FILE * idx_fp);
 *
 *    int write_thematic_index_header ( ThematicIndexHeader h ,
 *					     FILE *fp ) ;
 *
 *    int read_thematic_index_header ( ThematicIndexHeader *h ,
 *					    FILE *fp ) ;
 *
 *    int write_thematic_index_directory (
 *				       ThematicIndexHeader h ,
 *				       ThematicIndexDirectory *d,
 *				       int size,
 *				       FILE *fp ) ;
 *
 *    int write_gazetteer_index_directory (
 *				       ThematicIndexHeader h ,
 *				       ThematicIndexDirectory *d,
 *				       int size,
 *				       FILE *fp ) ;
 ...
 *				       char *idx_set );
 *
 *    int create_gazetteer_index (char *tablename,
 *				       char *idx_fname ,
 *				       char *columnname,
 *				       char *idx_set);
 *
...
/* changed unisgned long to int for use on dec */
static void *vpfmalloc(int size)
{
  void *ptr;
  ...
  if (ptr == NULL) {
    printf("VPFTIDX malloc:  Failed to allocate %lu bytes.\n",size);
    exit(1);
  }
...
static int read_gazetteer_index_directory(
                                   ThematicIndexDirectory **gid,
                                   ThematicIndexHeader     *gi,
                                   FILE                    *idx_fp);

static int read_thematic_index_header ( ThematicIndexHeader *h,
                                             FILE *ifp );

static int write_thematic_index_header ( ThematicIndexHeader h,
                                              FILE *ifp );

static int write_thematic_index_directory ( ThematicIndexHeader h,
                                          ThematicIndexDirectory *d,
                                          int idsize, /* size of data */
                                          FILE *ifp );

static int write_gazetteer_index_directory ( ThematicIndexHeader h,
                                          ThematicIndexDirectory *d,
                                          int idsize, /* size of data */
                                          FILE *ifp );
...
#define DIR_ON_DISK_INCR (2 * sizeof(int))

static int Icmpval( const void *elem1,
		    const void *elem2 )
{
...
static int Scmpval( const void *elem1,
		    const void *elem2 )
{
   ThematicIndexDirectory d1,d2;
...
static int Fcmpval( const void *elem1,
		    const void *elem2 )
{
   ThematicIndexDirectory d1,d2;
...
static int Dcmpval( const void *elem1,
		    const void *elem2 )
{
   ThematicIndexDirectory d1,d2;
...
static int Ccmpval( const void *elem1,
		    const void *elem2 )
{
   ThematicIndexDirectory d1,d2;
...
static int STRcmpval( const void *elem1,
		      const void *elem2 )
{
   ThematicIndexDirectory d1,d2;
...
static int bincmp( const void *elem1,
		   const void *elem2 )
{
   ThematicIndexDirectory d1,d2;
...
/* removed binoff, and bin from int decl, and created int decl for fread  */
int		bin,
		binoff;
  char           *buf ,
		 ch ,
  ...
  for ( i=0 ; i < (int)strlen(columnname); i++ )   /* copy into header structure */
    h.vpf_column_name[i] = toupper ( columnname[i] ) ;
  h.vpf_column_name[i] = '\0';

  h.index_type = 'I' ;		/* default */
  h.type_count = 1 ;
...
#define INCR_ID(loc,id) { \
  fwrite(&loc,sizeof(int),1,tmpfp); \
  j = d[loc].num_items; \
  fwrite(&j,sizeof(int),1,tmpfp); \
  d[loc].num_items++ ; \
...
/* Count up bins */
  ...
  case 'D':
	...
	if ( ! strcmp ( d[k].value.strval, datetemp ) )
      ...
      if ( k == h.nbins ) { 		/* New value in column */
	REALLOC_DIRECTORY ( k ) ;
	d[k].value.strval = (char *) vpfmalloc ( sizeof ( date_type ) + 2 ) ;
  ...
  default:
    ...
    if (h.column_type == 'T' && h.type_count > 1) {
       for ( i=0; i < h.nbins; i++ ) free(d[i].value.strval);
    }
    free ( d ) ;
  ...
  if ( write_thematic_index_header ( h, ifp ) == (int)NULL ) {
    vpf_close_table(&table);
    fclose(ifp);
    fclose(tmpfp);
    if (h.column_type == 'T' && h.type_count > 1) {
  ...
  switch (h.column_type) {
     case 'I':
	qsort(d,h.nbins,sizeof(ThematicIndexDirectory),Icmpval);
	break;
     case 'S':
	qsort(d,h.nbins,sizeof(ThematicIndexDirectory),Scmpval);
	break;
     case 'F':
	qsort(d,h.nbins,sizeof(ThematicIndexDirectory),Fcmpval);
	break;
     case 'R':
	qsort(d,h.nbins,sizeof(ThematicIndexDirectory),Dcmpval);
	break;
     case 'T':
	if (h.type_count == 1)
	   qsort(d,h.nbins,sizeof(ThematicIndexDirectory),Ccmpval);
	else
	   qsort(d,h.nbins,sizeof(ThematicIndexDirectory),STRcmpval);
	break;
     case 'D':
	qsort(d,h.nbins,sizeof(ThematicIndexDirectory),STRcmpval);
	break;
  ...
  if ( write_thematic_index_directory ( h, d, idsize, ifp ) == (int)NULL ) {
    vpf_close_table(&table);
    fclose(ifp);
    fclose(tmpfp);
    if (h.column_type == 'T' && h.type_count > 1) {
  ...
  qsort(d,h.nbins,sizeof(ThematicIndexDirectory),bincmp);
  ...
  for ( j=1; j <= table.nrows; j++ ) {
     ...
     if ( h.id_data_type == 'I' ) {
/* changed int to int within size of for dec */
	fseek(ifp, d[bin].start_offset + (binoff*sizeof(int)), 0);
	itemp = (int)j;
...
static ThematicIndexDirectory tidx_linear_search( char *value,
						  ThematicIndexHeader h,
						  FILE *ifp )
{
  int 			i , ival = 0;
  short int			sval = 0 , Match = (-1) ;
  float				fval = 0.0 ;
  ...
  if ( value ) 		/* search for pattern */
    ...
    case	'R':
      ...
      break ;
    case	'T':
      buf = (char *) vpfmalloc ( h.type_count+1 ) ;
      /* leave value as is */
      bufalloc = 1;
      break ;
    case 	'D':
...
static ThematicIndexDirectory tidx_binary_search( char *value,
					          ThematicIndexHeader h,
						  FILE *ifp )
{
  int 			i , ival = 0, left, right;
  ...
  if ( value ) 		/* search for pattern */
    ...
    case	'T':
      ...
      /* leave value as is */
      bufalloc = 1;
      recsize = h.type_count*sizeof(char) + 8;
      break ;
    case 	'D':
			       ...
			       char *value )
{
  ...
  if ( read_thematic_index_header ( &h, ifp ) == (int)NULL ) {
    fclose(ifp);
...
ThematicIndex open_thematic_index ( char *idxname, int storage )
{
  ...
  if ( read_thematic_index_header ( &idx.h, idx.fp ) == (int)NULL )
  ...
  if (storage == RAM) {
     idx.d = (ThematicIndexDirectory *)malloc(idx.h.nbins*
	 ...
	 switch ( idx.h.column_type ) {
	  ...
	  case 	'T':
	    if (idx.h.type_count == 1) {
	      ...
	      idx.d[i].value.strval = (char *)vpfmalloc(idx.h.type_count+1);
	    ...
	    break ;
	  case 	'D':
	    idx.d[i].value.strval = (char *)vpfmalloc(sizeof(date_type)+1);
	    Read_Vpf_Char ( idx.d[i].value.strval, idx.fp,
			    sizeof(date_type) );
	 ...
	 switch ( idx.h.column_type ) {
	  case 	'I':
	    qsort((void *)idx.d, idx.h.nbins, sizeof(ThematicIndexDirectory),
		  Icmpval) ;
	    break ;
	  case 	'S':
	    qsort((void *)idx.d, idx.h.nbins, sizeof(ThematicIndexDirectory),
				 ...
				 char *value )
{
  ...
  if (idx->d) {
     ...
     switch ( idx->h.column_type ) {
       case 'I':
	 memcpy( &dsearch.value.ival, value, sizeof(int) );
	 dptr = (ThematicIndexDirectory *)bsearch(
                        (void *)&dsearch, (void *)idx->d,
			idx->h.nbins, sizeof(ThematicIndexDirectory),
			Icmpval);
	 break ;
       case 'S':
	 ...
	 if (idx->h.type_count == 1) {
	   ...
	   dsearch.value.strval = (char *)vpfmalloc(idx->h.type_count+1);
	   memcpy( dsearch.value.strval, value, idx->h.type_count );
	   dsearch.value.strval[idx->h.type_count] = '\0';
	   dptr = (ThematicIndexDirectory *)bsearch(
                        (void *)&dsearch, (void *)idx->d,
			idx->h.nbins, sizeof(ThematicIndexDirectory),
			STRcmpval);
	   free(dsearch.value.strval);
	 }
	 break ;
       case 'D':
	 dsearch.value.strval = (char *)vpfmalloc(sizeof(date_type)+1);
				 ...
				 char *idx_set)
{
  ...
  gi.padding[1]  = ' ';
  gi.padding[2]  = ' ';
  set_byte_size  = ceil(t.nrows/8.0);
  ...
  gid = (ThematicIndexDirectory *) vpfmalloc(sizeof(ThematicIndexDirectory)
					  * gi.nbins);
  idx_bit_sets = (set_type *) vpfmalloc(sizeof(set_type) * gi.nbins);
  ...
  if (write_thematic_index_header(gi, idx_fp) == (int)NULL) {
    fclose(idx_fp);
    for (i = 0; i < gi.nbins; i++)
...
/* changed unisgned long to int for use on dec */
  ...
  if (read_thematic_index_header (&gi, idx_fp) == (int)NULL) {
    fclose(idx_fp);
    return query_set;
  }

  if (read_gazetteer_index_directory (&gid, &gi, idx_fp) == (int)NULL) {
    fclose(idx_fp);
    return query_set;
  }
  ...
  set_on(query_set);
  set_byte_size = ceil(gi.table_nrows/8.0);
  ...
  for (i = 0; i < query_len; i++) {
    ...
    if ( ! Read_Vpf_Char(xsect_set.buf, idx_fp, set_byte_size) ) {
      fprintf(stderr,"\nread_gazetteer_index: error reading index");
      set_off(query_set);
      set_nuke(&xsect_set);
      fclose(idx_fp);
      free(gid);
...
set_type search_gazetteer_index (ThematicIndex *idx, char *query_str )
{
  ...
  register int        query_len = strlen(query_str),
                      i,
                      j;
/* changed unsigned long to int for use on dec */
  int        set_byte_size;
  char                query_char;
  ...
  set_on(query_set);
  set_byte_size = ceil(idx->h.table_nrows/8.0);
  ...
  for (i = 0; i < gi->nbins; i++) {
    if ( ( ! Read_Vpf_Char(  &( (*gid)[i].value.cval ),   idx_fp, 1) ) ||
	 ...
	 ( ! Read_Vpf_Int(   &( (*gid)[i].num_items ),    idx_fp, 1) )) {
      fprintf (stderr,"\nread_gazetteer_index_directory: error reading dir");
      return (int) NULL;    }
  }
  return 1;
}
...
static int read_thematic_index_header ( ThematicIndexHeader *h, FILE *ifp ) 
{
  char *valid_index_types = "TGIB";
  ...
  if ( ! Read_Vpf_Char ( &h->sort, ifp, 1 ) )
    RWhimper() ;
  h->sort = toupper(h->sort);
  if ( ! Read_Vpf_Char ( &h->padding, ifp, 3 ) )
    RWhimper() ;

  /* Perform a simple (but theoretically not foolproof) validity check */
  if (!strchr(valid_index_types,toupper(h->index_type))) return (int) NULL;
  if (!strchr(valid_column_types,toupper(h->column_type))) return (int) NULL;
  if (!strchr(valid_id_data_types,toupper(h->id_data_type))) return (int) NULL;
  if (!strchr(valid_sort,toupper(h->sort))) return (int) NULL;
...
#define WWhimper() { \
   fprintf( stderr, "\nwrite_thematic_index_header: error reading header");\
   return (int) NULL ; }

static int write_thematic_index_header ( ThematicIndexHeader h, FILE *ifp ) 
{
  if ( fseek ( ifp, 0, 0 ) != 0 ) 	/* rewind, just in case */
    WWhimper() ;				/* error */
 ...
 *	idsize < input > == (int) size of each data element.
...
#define WTWhimper() { \
   fprintf( stderr,"\nwrite_thematic_index_directory: error reading header");\
   return (int) NULL ; }
...
#define WTGWhimper() {\
  fprintf( stderr,"\nwrite_gazetteer_index_directory: error writing header");\
  return (int) NULL ; }
