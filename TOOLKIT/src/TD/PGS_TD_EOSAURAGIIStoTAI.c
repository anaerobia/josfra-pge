...
FILENAME:   
   PGS_TD_EOSAURAGIIStoTAI.c

DESCRIPTION:
   This file contains the function PGS_TD_EOSAURAGIIStoTAI().
   This function converts EOS AURA (GIIS) spacecraft clock time in CCSDS day 
   segmented
...
HISTORY:
   ...
   05-Jun-1995  GTSK  Removed code duplicated elsewhere to calculate TAI.  This
		      ...
		      level functions.
   01-Dec-1999   AT   Used PGS_TD_EOSAMtoTAI to convert EOS AURA (GIIS format) 
                      to TAI time
...
TITLE:
   Convert EOS AURA (GIIS) Clock Time to TAI Time
...
INPUTS:
   NAME      DESCRIPTION
   ----      -----------
   scTime    EOS AURA (GIIS) clock time in CCSDS day segmented time code format 
             (CDS).  
...
NOTES:
   ...
   EOS AURA (GIIS) TIME FORMAT:

     For EOS-AURA (GIIS), the input spacecraft (s/c) clock time scTime is a 
     63-bit value
     ...
     to zero by EOS AURA (GIIS).  This makes the s/c time format used 
     by EOS AURA (GIIS) a variation of the CDS format (not supported by CCSDS).
...
#define FUNCTION_NAME "PGS_TD_EOSAURAGIIStoTAI()"

PGSt_SMF_status
PGS_TD_EOSAURAGIIStoTAI(                  /* converts EOS AURA (GIIS) s/c time to TAI */
    PGSt_scTime     scTime[8],      /* EOS AURA (GIIS) s/c time in CDC (almost) */
    PGSt_double     *secTAI93)      /* seconds since 12AM UTC 1-1-1993 */
{
    ...
    /* The first bit of the s/c time should be set to 0 for EOS AURA (GIIS).  
       Technically
