...
DESCRIPTION:
   This file contains the function PGS_TD_JDtoTJD().
...
HISTORY:
   07-Oct-1995  GTSK  Initial version
...
C:
   ...
   PGS_TD_JDtoTJD(
       ...
       PGSt_double  truncatedJulianDate[2])
...
OUTPUTS:
   Name                  Description               Units       Min   Max
   ----                  -----------               -----       ---   ---
   truncatedJulianDate    truncated Julian date    days        N/A   N/A

RETURNS:
   truncated Julian date (address of truncatedJulianDate).
...
PGS_TD_JDtoTJD(
    ...
    PGSt_double    truncatedJulianDate[2])
{
    /* The truncated Julian date is just the Julian date minus 2440000.5. */
    ...
    /* Since the toolkit Julian date format stores the Julian date as a half
       ...
       is the only component that needs to be modified.  The truncated Julian
       date returned will have an integer number for the first component (that
    ...
    truncatedJulianDate[0] = julianDate[0] - 2440000.5;
    truncatedJulianDate[1] = julianDate[1];
    
    return truncatedJulianDate;
}
