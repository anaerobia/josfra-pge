...
FILENAME:   
   PGS_TD_UTCtoEOSAURAGIRD.c

DESCRIPTION:
   This file contains the function PGS_TD_UTCtoEOSAURAGIRD().
   This function converts UTC in CCSDS ASCII Time Code A or CCSDS ASCII Time
   Code B  format to EOS AURA spacecraft clock time in CCSDS Unsegmented Time Code
...
TITLE:
   Convert UTC Time to EOS AURA spacecraft clock time
...
NOTES:
   ...
   EOS AURA TIME FORMAT:

     For EOS-AURA, the output spacecraft (s/c) clock time scTime is a 64-bit value
     ...
     1)  A 16-bit value (the first two elements of array
         ...
         for EOS AURA has a constant value of 10101110 (binary)
...
DETAILS:
   ...
   falls within the allowable range of the EOS AURA spacecraft clock.  If the
   above steps are all successful then the input time is decomposed into the
   appropriate octets of the EOS AURA spacecraft clock time (see 1)-3) above).
...
#define FUNCTION_NAME "PGS_TD_UTCtoEOSAURAGIRD()"

PGSt_SMF_status
PGS_TD_UTCtoEOSAURAGIRD(              /* convert UTC time to EOS-AURA s/c clock time */
    ...
    PGSt_scTime     scTime[8])  /* s/c clock time (in CUC) (output) */
{
    ...
    /* Call PGS_TD_UTCtoTAI to convert input UTC time to TAI time in
       ...
       TAI-UTC is required in the second octet of the P-field of the EOS AURA 
       spacecraft clock. */
	...
	PGS_SMF_SetDynamicMsg(PGSTD_E_DATE_OUT_OF_RANGE,
			      "The input UTC time "
			      "string is out of range for EOS AURA "
