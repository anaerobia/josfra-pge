...
DESCRIPTION:
   This file contains the function PGS_TD_UTCtoEOSAM().
   This function converts UTC in CCSDS ASCII time code A (or B) format
   to EOS AM spacecraft (s/c) clock time in CCSDS day segmented Time
...
TITLE:
   Convert UTC Time to EOS AM Clock Time
...
DESCRIPTION:
   This function converts UTC in CCSDS ASCII time code A (or B) format
   to EOS AM spacecraft (s/c) clock time in CCSDS Day Segmented (CDS) Time
...
NOTES:
   For EOS-AM, the output spacecraft (s/c) clock time scTime is a 63-bit value
   ...
   to zero by EOS AM.  This makes the s/c time format used by EOS AM a variation
...
DETAILS:
   ...
   EOS AM s/c clock time.  The input time values are then converted
   ...
   time is decomposed into the appropriate octets of the EOS AM
...
#define FUNCTION_NAME "PGS_TD_UTCtoEOSAM()"

PGSt_SMF_status
PGS_TD_UTCtoEOSAM(               /* convert UTC time to EOS-AM s/c clock time */
    ...
    unsigned char   scTime[8])   /* s/c clock time (in CDS) (output) */
{
    ...
    /* Call PGS_TD_julday() to determine the UTC Julian date of the input time
       ...
       range of the EOS AM s/c clock time.  Namely the input time should not
	...
	sprintf(msg,"%s%s%s","The date portion of the input UTC time ",
		             "string is out of range for EOS AM ",
