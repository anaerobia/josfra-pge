...
FILENAME:   
   PGS_TD_UTCtoEOSPMGIIS.c

DESCRIPTION:
   This file contains the function PGS_TD_UTCtoEOSPMGIIS().
   This function converts UTC in CCSDS ASCII time code A (or B) format
   to EOS PM (GIIS) spacecraft (s/c) clock time in CCSDS day segmented Time
...
TITLE:
   Convert UTC Time to EOS PM (GIIS) Clock Time
...
DESCRIPTION:
   This function converts UTC in CCSDS ASCII time code A (or B) format
   to EOS PM (GIIS) spacecraft (s/c) clock time in CCSDS Day Segmented (CDS) Time
...
OUTPUTS:    
   NAME       DESCRIPTION                         UNITS   MIN   MAX
   ----       -----------                         -----   ---   ---
   scTime     EOS PM (GIIS) clock time in CCSDS day      *see NOTES below
              segmented time code format (CDS).  

RETURNS:
   PGS_S_SUCCESS               successful execution
   PGS_E_DATE_OUT_OF_RANGE     input date is out of range of allowable
	                       times for the EOS PM (GIIS) spacecraft clock
...
NOTES:
   For EOS-PM (GIIS), the output spacecraft (s/c) clock time scTime is a 63-bit value
   ...
   to zero by EOS PM (GIIS).  This makes the s/c time format used by EOS PM (GIIS) a variation
...
DETAILS:
   ...
   EOS PM (GIIS) s/c clock time.  The input time values are then converted
   ...
   time is decomposed into the appropriate octets of the EOS PM (GIIS)
...
#define FUNCTION_NAME "PGS_TD_UTCtoEOSPMGIIS()"

PGSt_SMF_status
PGS_TD_UTCtoEOSPMGIIS(           /* convert UTC time to EOS-PM (GIIS) s/c clock time */
    ...
    unsigned char   scTime[8])   /* s/c clock time (in CDS) (output) */
{
    ...
    /* Call PGS_TD_julday() to determine the UTC Julian date of the input time
       ...
       range of the EOS PM (GIIS) s/c clock time.  Namely the input time should not
	...
	sprintf(msg,"%s%s%s","The date portion of the input UTC time ",
		             "string is out of range for EOS PM (GIIS) ",
