...
DESCRIPTION:
   This file contains the function PGS_TD_EOSAMtoTAI().
   This function converts EOS AM spacecraft clock time in CCSDS day segmented
...
TITLE:
   Convert EOS AM Clock Time to TAI Time
...
INPUTS:
   NAME      DESCRIPTION
   ----      -----------
   scTime    EOS AM clock time in CCSDS day segmented time code format (CDS).  
...
#define FUNCTION_NAME "PGS_TD_EOSAMtoTAI()"

PGSt_SMF_status
PGS_TD_EOSAMtoTAI(                  /* converts EOS AM s/c time to TAI */
    PGSt_scTime     scTime[8],      /* EOS AM s/c time in CDC (almost) */
    PGSt_double     *secTAI93)      /* seconds since 12AM UTC 1-1-1993 */
{
    ...
    /* The first bit of the s/c time should be set to 0 for EOS AM.  Technically
