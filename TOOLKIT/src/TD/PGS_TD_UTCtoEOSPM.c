...
DESCRIPTION:
   This file contains the function PGS_TD_UTCtoEOSPM().
   This function converts UTC in CCSDS ASCII Time Code A or CCSDS ASCII Time
   Code B  format to EOS PM spacecraft clock time in CCSDS Unsegmented Time Code
...
TITLE:
   Convert UTC Time to EOS PM spacecraft clock time
...
NOTES:
   ...
   EOS PM TIME FORMAT:
     ...
     For EOS-PM, the output spacecraft (s/c) clock time scTime is a 64-bit value
...
DETAILS:
   ...
   falls within the allowable range of the EOS PM spacecraft clock.  If the
   above steps are all successful then the input time is decomposed into the
   appropriate octets of the EOS PM spacecraft clock time (see 1)-3) above).
...
#define FUNCTION_NAME "PGS_TD_UTCtoEOSPM()"

PGSt_SMF_status
PGS_TD_UTCtoEOSPM(              /* convert UTC time to EOS-PM s/c clock time */
    ...
    PGSt_scTime     scTime[8])  /* s/c clock time (in CUC) (output) */
{
    ...
    /* Call PGS_TD_UTCtoTAI to convert input UTC time to TAI time in
       ...
       TAI-UTC is required in the second octet of the P-field of the EOS PM
       spacecraft clock. */
	...
	PGS_SMF_SetDynamicMsg(PGSTD_E_DATE_OUT_OF_RANGE,
			      "The input UTC time "
			      "string is out of range for EOS PM "
