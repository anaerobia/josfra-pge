...
DESCRIPTION:
   This file contains the function PGS_TD_TAItoUT1jd()
...
HISTORY:
   ...
   05-Jun-1995  GTSK  Removed code duplicated elsewhere to calculate UT1.  This
                      has been replaced with a call to the appropriate lower
                      level function--PGS_TD_UTCjdtoUT1jd().
   21-May-1997  PDN   Fixed Comments
   22-May-1997  PDN   Deleted code and reference to PGSTD_W_PRED_LEAPS
...
SYNOPSIS:
 ...
 FORTRAN:
      ...
      integer function pgs_td_taitout1(sectai93,jdut1)
...
#define FUNCTION_NAME "PGS_TD_TAItoUT1jd()"

PGSt_SMF_status
PGS_TD_TAItoUT1jd(                /* converts TAI time to UTC time */
    PGSt_double  secTAI93,        /* TAI (seconds) */
    PGSt_double  jdUT1[2])        /* UTC Julian date equivalent of input time */
{
    PGSt_double  jdUTC[2];        /* UTC Julian date equivalent of input time */
    PGSt_boolean onLeap;          /* in a leap second (T) or not (F) */
    ...
    returnStatus = PGS_TD_TAItoUTCjd(secTAI93,jdUTC);
    ...
    returnStatus1 = PGS_TD_UTCjdtoUT1jd(jdUTC, onLeap, jdUT1);
    ...
    /* Reset the message buffer if it has been overwritten with an incorrect
       value in the call to PGS_CSC_UTC_UT1Pole(). */
