...
FILENAME:   
   PGS_TD_EOSAURAGIRDtoTAI.c

DESCRIPTION:
   This file contains the function PGS_TD_EOSAURAGIRDtoTAI().
   This function converts EOS AURA spacecraft clock time in CCSDS 
   Unsegmented
...
TITLE:
   Convert EOS AURA Clock Time to TAI Time
...
INPUTS:
   NAME      DESCRIPTION
   ----      -----------
   scTime    EOS AURA clock time in CCSDS unsegmented time code format 
             (CUC).
...
NOTES:
   ...
   EOS AURA TIME FORMAT:
     ...
     For EOS-AURA, the output spacecraft (s/c) clock time scTime is a 
...
#define FUNCTION_NAME "PGS_TD_EOSAURAGIRDtoTAI()"

PGSt_SMF_status
PGS_TD_EOSAURAGIRDtoTAI(                /* converts EOS AURA s/c time to TAI */
    PGSt_scTime   scTime[8],      /* EOS AURA s/c time in CUC */
