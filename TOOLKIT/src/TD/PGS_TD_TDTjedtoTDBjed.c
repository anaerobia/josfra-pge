...
FILENAME:
  PGS_TD_TDTjedtoTDBjed.c

DESCRIPTION:
  This file contains the function PGS_TD_TDTjedtoTDBjed().
  This function converts TDT (Terrestrial Dynamical Time) as a Julian ephemeris
  date to TDB (Barycentric Dynamical Time) as a Julian ephemeris date.
...
HISTORY:
  24-Nov-1994 GTSK Initial version--code and comments adapted from work
                                    by PDN and AS
  18-Jun-1996 PDN  Fixed an integer constant "2" to be "2.0" 
...
TITLE:
   Convert TDT Julian ephemeris date to TDB Julian ephemeris date
...
FORTRAN:
      subroutine pgs_td_tdtjedtotdbjed(tdtjed,tdbjed)
...
EXAMPLES:
 C:
   PGSt_double       jedTDT[2]={2446147.5,0.0};
   PGSt_double       jedTDB[2];

   PGS_TD_TDTjedtoTDBjed(jedTDT,jedTDB);
...
FORTRAN:
      ...
      data jedtdt/2446147.5,0.0/

      call pgs_td_tdtjedtotdbjed(jedtdt,jedtdb)
...
DETAILS:
    ...
    This function requires as input TDT (Terrestrial Dynamical Time) as a Julian
    date. Using TDT the mean anomaly (g) of the earth is calculated using the
    following formula:
...
PGSt_double *
PGS_TD_TDTjedtoTDBjed(      /* converts TDT as a Julian date to TDB as a Julian
			       date */
    ...
    PGSt_double jedTDB[2])  /* TDB time as a Julian ephemeris date */
{
    ...
    jedTDB[0] = jedTDT[0];
    jedTDB[1] = jedTDT[1] +
                (0.001658 * sin(g) + 0.000014 * sin(2.0*g))/SECONDSperDAY;
    ...
    /* IMPORTANT: this test must not be in an else clause dependent on the
       ...
       test to succeed and then set jedTDB to 1.0 which is undesireable. */
