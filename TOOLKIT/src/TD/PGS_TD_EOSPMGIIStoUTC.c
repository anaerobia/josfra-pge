...
FILENAME:   
   PGS_TD_EOSPMGIIS_to_UTC.c

DESCRIPTION:
   This function converts EOS PM (GIIS) spacecraft clock time in CCSDS
...
HISTORY:
   ...
   01-Dec-1999   AT   Used PGS_TD_EOSAMtoUTC to convert EOSPM (GIIS) time to 
                      UTC time
END_FILE_PROLOG:
*******************************************************************************/
...
TITLE:
   Convert EOS PM (GIIS) Clock Time to UTC Time
...
DESCRIPTION:
   This function converts EOS PM (GIIS) spacecraft clock time in
   platform-dependent format to UTC in CCSDS ASCII time code A format.
 
INPUTS:
   NAME      DESCRIPTION                  UNITS    MIN          MAX
   ----      -----------                  -----    ---          ---
   scTime    EOS PM (GIIS) clock time in CCSDS     ** see NOTES below **
             segmented time code
	     format (CDS).  
...
NOTES:
   ...
   EOS PM (GIIS) TIME FORMAT:
     ...
     For EOS-PM (GIIS), the input spacecraft (s/c) clock time scTime is a 63-bit value
     ...
     to zero by EOS PM (GIIS).  This makes the s/c time format used by EOS PM (GIIS) a
...
#define FUNCTION_NAME "PGS_TD_EOSPMGIIStoUTC()"

PGSt_SMF_status
PGS_TD_EOSPMGIIStoUTC(              /* converts EOS PM (GIIS) s/c time to ASCII UTC */
    PGSt_scTime   scTime[8],    /* EOS PM (GIIS) s/c time in CCSDS unseg. time code */
