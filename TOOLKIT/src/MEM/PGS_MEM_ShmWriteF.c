...
DESCRIPTION:
   This file contains the function PGS_MEM_ShmWriteF().
   This function copies the contents of a user allocated (may be dynamically or
   statically allocated) memory area into shared memory.
...
DESCRIPTION:
      This function copies the contents of a user allocated (may be dynamically
      or statically allocated) memory area into shared memory.  This function is
      is meant to be used by FORTRAN (77/90) users who cannot take advantage of
      the C shared memory tool PGS_MEM_ShmAttach() and PGS_MEM_ShmDetach().

INPUTS:        
    NAME         DESCRIPTION
    ----         -----------
    mem_ptr      pointer to memory area the contents of which are to be written
                 shared memory

    size         size (in bytes) of the memory area to copy into shared memory
...
!  the contents of shm_buffer have been written to the shared memory
!  area which can be accessed by a subsequent process
...
DETAILS:
    ...
    to the shared memory area and write the contents of the users memory area
    to the shared memory area OVERWRITING whatever was previously in the shared
    memory area.  This function will then detach the shared memory from the
    current process.  Before exiting from the PGE, the system will make sure
    ...
    PGSt_integer     size)            /* size of memory area to write to */
{
    ...
    {
	/* copy contents of input buffer to shared memory and then detach shared
	   memory */

	memcpy(shared_mem_ptr, mem_ptr, size);
