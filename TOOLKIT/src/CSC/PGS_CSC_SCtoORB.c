...
DESCRIPTION:
   This file contains the function PGS_CSC_SCtoORB().
...
HISTORY:
   ...
   20-Mar-1994 GTSK Replaced multiple calls to malloc() with single call to
                    PGS_MEM_Calloc().
   15-Jun-1995 GTSK Reworked algorithm which was calculating ORB to SC!
...
FORTRAN:
      ...
      integer function pgs_csc_sctoorb(spacecrafttag,numvalues,asciiutc,
...
DESCRIPTION:
   Transforms vector in Spacecraft reference frame to a vector in Orbital 
   reference frame.
...
OUTPUTS:
   Name          Description                     Units       Min        Max
   ----          -----------                     ----        ---        ---
   posORB        Coordinates or unit vector      meters      N/A        N/A
                 components in orbital
                 reference frame                   
...
C:
   ...
   returnStatus = PGS_CSC_SCtoORB(PGSd_TRMM,numValues,asciiUTC,offsets,posSC,
                                  posORB)
...
#define  FUNCTION_NAME "PGS_CSC_SCtoORB()"

PGSt_SMF_status
PGS_CSC_SCtoORB(                    /* converts from Spacecraft coordinate 
                                       system to Orbital coordinate system */ 
...
{    
    ...
    /* Allocate space for scratch arrays.  This includes qualityFlags, 
       eulerAngle, angularVelocity, attitQuat. */
    ...
    /* Assign space for attitQuat - Spacecraft coordinate system to ECI 
       rotation quaternion. */

    attitQuat = (PGSt_double (*)[4]) (qualityFlags + maxValues);

    /* Assign space for, eulerAngle - Spacecraft Attitude Euler
    ...
    for(cnt1=0;cnt1 < maxValues;cnt1++)
    {
            ...
            ((qualityFlagsMasks[1] & qualityFlags[cnt1][1]) != 0))
        {
            ...
            {
                posORB[cnt1][iXYZ] = PGSd_GEO_ERROR_VALUE;
            }
            if (returnStatus == PGS_S_SUCCESS)
            {
                returnStatus = PGSCSC_W_BAD_TRANSFORM_VALUE;
        ...
        /* call PGS_CSC_EulertoOrbit_Quat to get the quaternion defining the
           rotation from the spacecraft to the orbital (or tipped (TRMM))
           reference frame */
        ...
        /* call PGS_CSC_quatRotate to transform the vector from SC to the
           rotated Orbital coordinate system where the rotation is 
           defined by a quaternion */
        
        returnStatus1 = PGS_CSC_quatRotate(orbitalQuat, posSC[cnt1],
                                           posORB[cnt1]);
