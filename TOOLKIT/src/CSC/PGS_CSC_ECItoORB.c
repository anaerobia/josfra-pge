...
DESCRIPTION:
   This file contains the function PGS_CSC_ECItoORB().
   This function transforms a vector in the ECI Coordinate system to a vector 
...
HISTORY:
   ...
   12-Feb-1999  PDN   Deleted references to predicted leap seconds, which 
                      were used in early I&T work only.
...
TITLE: 
   Transform from ECI Frame to ORB Frame
...
FORTRAN:
      ...
      integer function
      pgs_csc_ecitoorb(spacecrafttag,numvalues,asciiutc,offsets,
...
DESCRIPTION:
   Transforms vector in ECI coordinate system to vector in Orbital coordinate
   system.
...
C:
   ...
   returnStatus = PGS_CSC_ECItoORB (TRMM,numValues,asciiUTC,offsets,positionECI,
                                    positionORB)
...
FORTRAN:
   ...
   10 continue
      ...
      returnstatus = pgs_csc_ecitoorb(trmm, numvalues, asciiutc, offsets,
     >                                positioneci, positionorb)
...
DETAILS:
   ...
   translation to the orbital coordinate system origin is performed first and
...
#define  FUNCTION_NAME  "PGS_CSC_ECItoORB()"

PGSt_SMF_status
PGS_CSC_ECItoORB(                       /* transform vector from ECI to ORB */
    ...
    PGSt_double     positionECI[][3],   /* array of ECI vectors input */
    PGSt_double     positionORB[][3])   /* array of equiv. ORB vectors output */
{
    PGSt_double     (*scPosECI)[3];       /* spacecraft ECI position (meters) */
    PGSt_double     (*scVelECI)[3];     /* spacecraft ECI velocity (m/s) */
    PGSt_double     quatECItoORB[4];    /* quat. defining ECI to ORB rotation */
    PGSt_double     norm;               /* norm of input vectors */
    ...
    /* Assign space for scPosECI - Spacecraft ECI position vector and
    ...
    /* for each time requested, transform the corresponding input vector from
       the ECI to the ORB coordinate system */

    for (cnt=0;cnt<maxValues;cnt++)
    {
	/* don't process bad values from call to PGS_EPH_EphemAttit(), set the
	   corresponding positionORB vector components to PGSd_GEO_ERROR_VALUE.
	   */
	...
	if ((qualityFlagsMasks[0] & qualityFlags[cnt][0]) != 0)
	{
	    ...
	    {
		positionORB[cnt][iXYZ] = PGSd_GEO_ERROR_VALUE;
	    }
	    if (returnStatus == PGS_S_SUCCESS)
	    {
		returnStatus = PGSCSC_W_BAD_TRANSFORM_VALUE;
	...
	/* Get the quaternion that defines the rotation from the ECI to the ORB
	   coordinate system.  If this is successful, translate the ECI vector
	   input from the Earth center to the origin of the ORB coordinate
	   system.  If unsuccessful set all components of return vector to
	   PGS_GEO_ERROR_VALUE and process next vector. */

	returnStatus1 = PGS_CSC_getECItoORBquat(scPosECI[cnt], scVelECI[cnt],
						quatECItoORB);
	switch (returnStatus1)
	{
	  case PGS_S_SUCCESS:
	  ...
	  case PGS_E_TOOLKIT:
	    ...
	    for (iXYZ=0;iXYZ<3;iXYZ++)
		positionORB[cnt][iXYZ] = PGS_GEO_ERROR_VALUE;
	    continue;
	  default:
	    PGS_MEM_Free(memPtr);
	    PGS_SMF_SetUnknownMsg(returnStatus1,FUNCTION_NAME);
	...
	/* Compute the dot product of the input vector positionECI.  This is
	   needed to see if the input vector is a unit vector.  The dot product
	   is obviously NOT the norm but since it is the square of the norm and
	   we are only checking to see if it is 1.0, we'll cheat and pretend
	   the dot product is the norm since 1.0^2 is 1.0! */
	
	norm = PGS_CSC_dotProduct(positionECI[cnt],positionECI[cnt],3);
	
	/* Check if the norm of the input vector is between 0.99999 and 1.00001.
	   If it is, then only do rotation, otherwise do a translation and then
	   a rotation.  Either way, correct for aberration. */
	...
	if (norm < 0.99999 || norm > 1.00001)
	{
	    ...
	    for (iXYZ=0;iXYZ<3;iXYZ++)
		positionORB[cnt][iXYZ] = positionECI[cnt][iXYZ] -
                                         scPosECI[cnt][iXYZ];

	    /* if length of input vector AFTER translation is greater than
	       120 meters then do aberration correction, otherwise assume we
	       are looking at a piece of the spacecraft (no need to correct for
	       aberration) */
	    ...
	    {
		for (iXYZ=0;iXYZ<3;iXYZ++)
		    positionORB[cnt][iXYZ] = positionORB[cnt][iXYZ] + 
			                     norm*vDivc[iXYZ];  
	    }
	}    
	else
	{
	    for (iXYZ=0;iXYZ<3;iXYZ++)
		positionORB[cnt][iXYZ] = positionECI[cnt][iXYZ] + vDivc[iXYZ];

	    norm = PGS_CSC_Norm(positionORB[cnt]);

	    for (iXYZ=0;iXYZ<3;iXYZ++)
		positionORB[cnt][iXYZ] = positionORB[cnt][iXYZ]/norm;
	}
	
	/* rotate the vector that is currently centered at the ORB coordinate
	   system origin from its ECI coordinates to its ORB coordinates */
	
	returnStatus1 = PGS_CSC_quatRotate(quatECItoORB,positionORB[cnt],
					   positionORB[cnt]);
