...
DESCRIPTION:
   This file contains the function PGS_CSC_ECItoECR().
   This function rotates an array of 6-vectors from ECI (J2000) coordinates to
   ECR (of date) coordinates.  The rotation is done in 3 parts:  precession, 
   nutation, Earth rotation about the nutated axis.
...
HISTORY:
   10-Feb-1994 PDN  Designed				
   24-Feb-1994 PDN  Created
   25-Feb-1994 PDN  Modified to implement polar motion, provide for efficiency
                    in cases of repeated calls with the same time value 
   08-Mar-1994 PDN  Modified to incorporate a new TAI routine that gives a 
                    smooth transition at leap seconds
   08-Mar-1994 PDN  Modified to correct axes in polar motion
   09-Sep-1994 AS   Updated to accept time offsets,updated calling sequences,
                    updated types, updated variable names, updated error 
                    messages 
   14-Sep-1994 AS   Updated prolog to conform to latest PGS/ECS standards
   16-Sep-1994 AS   Replaced code with call to PGS_TD_TAItoUT1pole()
   19-Sep-1994 AS   Made changes in error handling
   22-Sep-1994 AS   Replaced internal norm function with PGS_CSC_Norm()
   27-Sep-1994 AS   Changed error handling from call to PGS_TD_TAItoUT1pole() 
   23-May-1995 GTSK Rewritten to improve efficiency and readability
   ...
   21-Jun-1996 PDN  Deleted test for obsolete return; fixed related comments
   24-Aug-1997 PDN  Replaced Earth rotation rate 0.000072921151467 rad/s from
                    Explan. Suppl. to Astron Almanac with IERS value
...
FORTRAN:
      ...
      integer function pgs_csc_ecitoecr(numvalues,asciiutc,offsets,
     >                                  posveleci,posvelecr)
      integer          numvalues
...
OUTPUTS:
   posvelECR[6]  Vector after being 
                 transformed to ECR 
                 of date
...
C:
   ...
   returnStatus = PGS_CSC_ECItoECR(numValues,asciiUTC,offsets,posvelECI,
                                   posvelECR)
...
NOTES:
   TIME ACRONYMS:
    ...
    TIME BOUNDARIES:
     ...
     if the U.S. Naval Observatory later improves its solution). Here follows
...
DETAILS:
   This function rotates a 6-vector in ECI of 3 position coordinates and 
   3 velocity components from J2000 to the epoch defined by asciiUTC (plus any
   offset).  The rotation is in four parts (with each is indicated the number of
   rotations):

   precession   (3)
   nutation     (3)
   diurnal Earth rotation about the nutated axis (1)
   polar motion (2)

   The sequence of operations for ECI -> ECR is:

        1. get TAI from UTC and leap seconds
        2. get precession and nutation and TDB from TAI
        3. get UT1-UTC and polar motion from a utility that reads         
           tabulated values                                               
        4. get Greenwich mean sidereal time from UT1 using algorithm from 
           Astron. Almanac                                                
        5. get Greenwich apparent sidereal time "gast" from Greenwich mean
           time and nutation
        6. precess and nutate the vector
        7. rotate the vector about the N rotation pole by the angle gast 
           in radians
        8. adjust to the true geographic pole by polar motion (from Step 2)

GLOBALS:
     PGSg_TSF_CSCECItoECRsetup_msg
     PGSg_TSF_CSCECItoECRlastTAI93
     PGSg_TSF_CSCECItoECRjedTDB
     PGSg_TSF_CSCECItoECRjedTDT
     PGSg_TSF_CSCECItoECRdvnut
     PGSg_TSF_CSCECItoECRearthrot
     PGSg_TSF_CSCECItoECRxpol
     PGSg_TSF_CSCECItoECRypol
     PGSg_TSF_CSCECItoECRsetupStatus

FILES:
   This tool accesses file leapsec.dat and utcpole.dat  
...
#define FUNCTION_NAME "PGS_CSC_ECItoECR()"

PGSt_SMF_status 
PGS_CSC_ECItoECR(                  /* transforms from ECI reference frame to 
                                      ECR ref. frame */
    ...
    PGSt_double  posvelECI[][6],   /* input vector: position (m) and velocity 
                                     (m/s) in ECI */
    PGSt_double  posvelECR[][6])   /* the answer - vector rotated to ECR */ 
{    
    ...
    static PGSt_double xpol[2]={0.,0.};   /* x offset of polar wander in seconds
					     of arc on input; converted to 
					     radians in place */
    static PGSt_double ypol[2]={0.,0.};   /* y offset of polar wander in seconds
					     of arc on input; converted to
					     radians in place */
   ...
   /*  Declare variables used for THREADSAFE version to replace statics 
    ...
    char  setup_msgTSF[PGS_SMF_MAX_MSG_SIZE];

               /*  Globals     originals in PGS_TSF_SetupCSC.c */

    extern char PGSg_TSF_CSCECItoECRsetup_msg[][PGS_SMF_MAX_MSG_SIZE];
    extern PGSt_SMF_status PGSg_TSF_CSCECItoECRsetupStatus[];
    extern PGSt_double PGSg_TSF_CSCECItoECRlastTAI93[];
    extern PGSt_double PGSg_TSF_CSCECItoECRjedTDB[][2];
    extern PGSt_double PGSg_TSF_CSCECItoECRjedTDT[][2];
    extern PGSt_double PGSg_TSF_CSCECItoECRdvnut[][4];
    extern PGSt_double PGSg_TSF_CSCECItoECRearthrot[][2];
    extern PGSt_double PGSg_TSF_CSCECItoECRxpol[][2];
    extern PGSt_double PGSg_TSF_CSCECItoECRypol[][2];
    ...
    setupStatusTSF = PGSg_TSF_CSCECItoECRsetupStatus[masterTSFIndex];
    strcpy(setup_msgTSF,PGSg_TSF_CSCECItoECRsetup_msg[masterTSFIndex]);
    lastTAI93TSF = PGSg_TSF_CSCECItoECRlastTAI93[masterTSFIndex];
    for (x=0; x<2; x++)
    {
        jedTDBTSF[x] = PGSg_TSF_CSCECItoECRjedTDB[masterTSFIndex][x];
        jedTDTTSF[x] = PGSg_TSF_CSCECItoECRjedTDT[masterTSFIndex][x];
        earthrotTSF[x] = PGSg_TSF_CSCECItoECRearthrot[masterTSFIndex][x];
        xpolTSF[x] = PGSg_TSF_CSCECItoECRxpol[masterTSFIndex][x];
        ypolTSF[x] = PGSg_TSF_CSCECItoECRypol[masterTSFIndex][x];
    }
    for (x=0; x<4; x++)
    {
        dvnutTSF[x] = PGSg_TSF_CSCECItoECRdvnut[masterTSFIndex][x];
    }
#endif
    ...
    {
	/* Initialize ECR values to ECI values, because the nutation and
	   precession are "in place" functions */

	for (cnt2=0; cnt2 < 6; cnt2++) 
	    posvelECR[cnt1][cnt2] = posvelECI[cnt1][cnt2];
	...
	if (lastTAI93 != secTAI93) /* BEGIN setup */
	    ...
	    /* Save the return status from the above call to 
	       ...
	       reported. */
            ...
            /* Threadsafe Protect: setupStatus, setup_msg, xpol,ypol, jedTDB 
               and dvnut,      reassign their values for next use */

	    if ((setupStatusTSF=returnStatus) != PGS_S_SUCCESS)
	    {
		strcpy(setup_msgTSF,msg);
                strcpy(PGSg_TSF_CSCECItoECRsetup_msg[masterTSFIndex],
                                                        setup_msgTSF);
	    }		
            PGSg_TSF_CSCECItoECRsetupStatus[masterTSFIndex] = setupStatusTSF;
	    
	    /* Change the Polar Motion units from seconds of arc to 
	       radians  */
	
	    xpolTSF[0] = xpole/206264.806;
            PGSg_TSF_CSCECItoECRxpol[masterTSFIndex][0] = xpolTSF[0];
	    ypolTSF[0] = ypole/206264.806;
            PGSg_TSF_CSCECItoECRypol[masterTSFIndex][0] = ypolTSF[0];
	    ...
	    PGS_CSC_quickWahr(secTAI93, jedTDBTSF, dvnutTSF);
            for (x=0; x<4; x++)
            {
                PGSg_TSF_CSCECItoECRdvnut[masterTSFIndex][x] = dvnutTSF[x];
            }
            for (x=0; x<2; x++)
            {
                PGSg_TSF_CSCECItoECRjedTDB[masterTSFIndex][x] = jedTDBTSF[x];
            }
	    ...
	    /* determine Earth rotation angle and rate about true N pole of
	       date; first component is GAST, the second is 0.000072921151,
	    ...
	    earthrotTSF[1] = EARTH_ROTATE_RATE;
            for (x = 0; x<2; x++)
            {
                PGSg_TSF_CSCECItoECRearthrot[masterTSFIndex][x] = 
                                                         earthrotTSF[x];
            }
	    ...
	    PGS_TD_TAItoTAIjd(secTAI93, jedTDTTSF);
            for (x=0; x<2; x++)
            {
                PGSg_TSF_CSCECItoECRjedTDT[masterTSFIndex][x] = jedTDTTSF[x];
            }
	    ...
	    lastTAI93TSF = secTAI93;    
            PGSg_TSF_CSCECItoECRlastTAI93[masterTSFIndex] = lastTAI93TSF;
	...
	/* actually check dot product of vector with itself (rather than norm)
	   ...
	   500,000,000 (see comment above) squared */
	...
	else
	{
		...
		posvelECR[cnt1][cnt2] = 0.0;
	}
	
	/* The input vector still has not been touched.  Now apply precession
	   and nutation to bring it from J2000 to the Celestial coordinates of
	   date */
	...
	PGS_CSC_precs2000(threeOr6, jedTDTTSF, PGS_TRUE, posvelECR[cnt1]);
	
	/* nutate the vector to pole of date (jedTDT) */
	
	PGS_CSC_nutate2000(threeOr6, jedTDTTSF, dvnutTSF,
                                    PGS_TRUE, posvelECR[cnt1]);
	...
	pgs_rotate(posvelECR[cnt1], earthrotTSF, 3, posvelECR[cnt1]);
	
	/*  Rotate to compensate polar motion in y (see A.A. p. B60, 
	    R1(-y) term)  */
	
	pgs_rotate(posvelECR[cnt1], ypolTSF, -1, posvelECR[cnt1]);

	/*  Rotate to compensate polar motion in x (see A.A. p. B60, 
	    R2(-x) term)  */

	pgs_rotate(posvelECR[cnt1], xpolTSF, -2, posvelECR[cnt1]);
#else
	PGS_CSC_precs2000(threeOr6, jedTDT, PGS_TRUE, posvelECR[cnt1]);
	
	/* nutate the vector to pole of date (jedTDT) */
	
	PGS_CSC_nutate2000(threeOr6, jedTDT, dvnut, PGS_TRUE, posvelECR[cnt1]);
	...
	pgs_rotate(posvelECR[cnt1], earthrot, 3, posvelECR[cnt1]);
	
	/*  Rotate to compensate polar motion in y (see A.A. p. B60, 
	    R1(-y) term)  */
	
	pgs_rotate(posvelECR[cnt1], ypol, -1, posvelECR[cnt1]);

	/*  Rotate to compensate polar motion in x (see A.A. p. B60, 
	    R2(-x) term)  */

	pgs_rotate(posvelECR[cnt1], xpol, -2, posvelECR[cnt1]);
#endif
    }
