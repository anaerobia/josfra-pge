...
DESCRIPTION:
   This file contains the function PGS_CSC_ECItoSC().
...
HISTORY:
   ...
   01-Sep-1994 AS   Updated to accept time offsets,calling sequences updated, 
                    types updated, variable names updated,error messages 
                    updated 
   04-Sep-1994 AS   Include aberration correction                
   15-Sep-1994 AS   Modified prologs and comply with latest ECS/PGS standards. 
   21-Sep-1994 AS   Fixed up errors from code inspection
   26-Sep-1994 AS   Modified to use function PGS_CSC_Norm to calculate the norm
                    of a vector 
   ...
   12-Jul-1995 PDN  Fixed items in prolog
   12-Feb-1999 PDN  Fixed nomenclature consistent with AM1 and later spacecraft
                    note that the function is also OK for TRMM; for TRMM
...
FORTRAN:
      ...
      integer function pgs_csc_ecitosc(spacecrafttag,numvalues,asciiutc,
...
DESCRIPTION:
   Transforms vector in ECI coordinate system to vector in Spacecraft 
   coordinate system.
...
INPUTS:
   ...
   posECI        Coordinates or unit         N/A        N/A  
...
C:
   ...
   returnStatus = PGS_CSC_ECItoSC (PGSd_TRMM,numValues,asciiUTC,offsets,posECI,
                                   posSC)
...
DETAILS:
   ...
   The function first gets the position vector and attitude quaternions
   for the specified spacecraft at the specified times.

   Next the function checks the input vector to see if it is a unit 
   vector. If so, it is assumed that the user wishes only to transform its 
   direction.  If not, it is assumed that the vector locates some point of 
   interest (for example, a TDRSS satellite, or a lookpoint). Thus, for 
   that case a translation to the spacecraft center is performed first and 
   then a rotation. Aberration correction is also performed in both cases,
   except in the second case, points within 120 m of spacecraft center are
   not aberrated. 
...
#define PGS_RE          6378130.0   /* radius of Earth (meters) */
#define SPEED_OF_LIGHT  299792458.0 /* speed of light (m/s) */

/* name of this function */

#define FUNCTION_NAME  "PGS_CSC_ECItoSC()"

PGSt_SMF_status
PGS_CSC_ECItoSC(                    /* converts from ECI coordinate system to 
    ...
    PGSt_double	 posSC[][3])        /* coordinates in SC reference frame */ 
{
    ...
    PGSt_double  (*angularVelocity)[3];
                                    /* J2000 anguler velocity proj. on the 
                                       body axes in the order roll,pitch,yaw */
    ...
    PGSt_double  copyECI[3];        /* copy of posECI[3] to protect against 
                                       over-writing */
    ...
    /* Assign space for positionECI - Spacecraft ECI position vector, velocityECI
       ...
       Angles, angularVelocity - Spacecraft J2000 angular velocity projected on
       the body roll, pitch, and yaw axes, in that order (except for TRMM; in 
       that case these are the Euler angle rates for yaw, pitch and roll) . */
	...
	/*** Compute the norm of the input vector posECI ***/
	...
	/* Set up scratch vector to protect input */
	
	for (cnt2=0; cnt2 < 3; cnt2++) 
	    copyECI[cnt2] = posECI[cnt1][cnt2];
	
	/* Check if the norm of the input vector is between 0.99999 and 1.00001,
	   if it is, then do aberration correction and rotation Otherwise do a
	   translation and then a rotation. */
        
	if (norm >= 0.99999 && norm <= 1.00001)
	{
	    /* do the aberration correction */
	    
	    for(cnt2=0;cnt2 < 3;cnt2++)
		copyECI[cnt2] = copyECI[cnt2] + vDivc[cnt2];
	    
	    norm = PGS_CSC_Norm(copyECI);
            
	    for(cnt2=0;cnt2 < 3;cnt2++)
		copyECI[cnt2] = copyECI[cnt2]/norm;
	}    
	else 
	{
	    /* Check if the norm is <= 0.98 * (radius of earth) If it is, return
	      warning message indicating point is below the surface of the
	      earth, possible but unlikely to be what was intended */
	    ...
	    for(cnt2=0;cnt2 < 3;cnt2++)
		copyECI[cnt2] -= positionECI[cnt1][cnt2]; 
	    
	    /* if length of input vector after translation is greater than
	       120 meters then do aberration correction */
	    ...
	    {
		for(cnt2=0;cnt2 < 3;cnt2++) 
		    copyECI[cnt2] = copyECI[cnt2] + norm*vDivc[cnt2];  
	    }
	    
	} 
	
	/* convert the quaternions to those for an inverse rotation, since
	   PGS standards are that the usual quaternion takes a vector 
	   from SC to ECI */
	
	for (cnt2 = 1;cnt2 < 4; cnt2++)
	    attitQuat[cnt1][cnt2] =  -attitQuat[cnt1][cnt2];
	
	/* call PGS_CSC_quatRotate to transform the vector from ECI to the
	   rotated SC coordinate system where the rotation is defined by 
	   a quaternion */
	
	returnStatus1 = PGS_CSC_quatRotate(attitQuat[cnt1],copyECI,posSC[cnt1]);
