...
FILENAME:
   PGS_CSC_Earthpt_FixedFOV.c

DESCRIPTION:
   This file contains the function PGS_CSC_Earthpt_FixedFOV().
...
AUTHOR:
   Edmund Larson / Applied Research Corporation
   Peter D. Noerdlinger / Space Applications Corporation

HISTORY:
   29-Apr-1996  EL  Initial version--based on code in PGS_CSC_Earthpt_FOV.c
   24-Aug-1997  PDN Deleted unneeded return for PGS_SMF_SetDynamicMsg
...
FORTRAN:
      ...
      integer function
     >pgs_csc_earthpt_fixedfov(numvalues,asciiutc,offsets,spacecrafttag,
...
C:
   ...
   PGSt_double        altitude;
   PGSt_integer       numFOVperimVec;
   PGSt_double        inFOVvector[3] = {0.0,0.0,100.0};
...
NOTES:
   ...
   The FOV is always specified in SC coordinates. The instrument is assumed 
   fixed to the SC, so the function uses the same FOV description always. For 
   scanning instruments, use PGS_CSC_Earthpt_FOV().
   numFOVperimVec should be at least 3. The tool determines if the earth point
   lies within the perimeter defined by the vectors perimFOVvectors[][3]. 
   The first index in C (last in Fortran) must be sequential around the FOV 
   perimeter. If the altitude is unknown, use zero.

   The vector inFOVvector[3] must be defined in SC coordinates and must lie 
   within the FOV. It is necessary for the user to supply a vector 
   within the FOV because on the surface of a sphere, a closed
...
#define FUNCTION_NAME "PGS_CSC_Earthpt_FixedFOV()"

PGSt_SMF_status
PGS_CSC_Earthpt_FixedFOV(              /* returns a flag or flags indicating 
    ...
    PGSt_double  perimFOV_vectors[][3],/* vectors in SC coords defining FOV */
    ...
    PGSt_double  sctoEarthptVec[][3])  /* vector to Earth point in SC coords */
{
	...
	for (cnt2 = 0; cnt2 < 3; cnt2++)
	{
		perimFOV_vecCopy[counter1][cnt2] = 
		  perimFOV_vectors[counter1][cnt2];
	}
    }

    /* normalize the in field of view vector */
    
    norm = PGS_CSC_Norm(inFOVvector);
    
    if (fabs(norm) > EPS_64)
      for (cnt2 = 0; cnt2 < 3; cnt2++)
	inFOVvecNorm[cnt2] = inFOVvector[cnt2] / norm;
    else
    {	
	if (returnStatus == PGS_S_SUCCESS)
	{
